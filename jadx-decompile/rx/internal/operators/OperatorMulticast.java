...
public final class OperatorMulticast<T, R> extends ConnectableObservable<R> {
    ...
    class AnonymousClass3 extends Subscriber<R> {
        final /* synthetic */ Subscriber val$s;

        AnonymousClass3(Subscriber x0, Subscriber subscriber) {
    ...
    public void connect(Action1<? super Subscription> connection) {
        synchronized (this.guard) {
            ...
            gs.set(Subscriptions.create(new AnonymousClass2(gs)));
            this.guardedSubscription = (Subscription) gs.get();
            for (Subscriber<? super R> s : this.waitingForConnect) {
                subject.unsafeSubscribe(new AnonymousClass3(s, s));
            }
