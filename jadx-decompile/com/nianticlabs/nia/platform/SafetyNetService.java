...
public class SafetyNetService extends ContextService {
    ...
    class AnonymousClass2 implements Runnable {
        ...
        class AnonymousClass1 implements ResultCallback<AttestationResult> {
            ...
            public void onResult(AttestationResult result) {
                ...
                if (result.getStatus().isSuccess()) {
                    ...
                    if (SafetyNetService.checkResult(jwsResult)) {
                        SafetyNetService.this.attestResponse(AnonymousClass2.this.val$nonce, jwsResult);
                        return;
                    } else {
                        SafetyNetService.this.attestResponse(AnonymousClass2.this.val$nonce, null);
                        return;
                    }
                }
                SafetyNetService.this.attestResponse(AnonymousClass2.this.val$nonce, null);
            ...
            if (SafetyNetService.this.googleApiState == GoogleApiState.STARTED) {
                ...
                synchronized (SafetyNetService.this.lock) {
                    if (((PendingResult) SafetyNetService.this.requestMap.get(nonceBuffer)) == null) {
                        ...
                        result.setResultCallback(new AnonymousClass1(nonceBuffer));
                    }
                }
                return;
            }
    ...
    public void attest(byte[] nonce) {
        ContextService.runOnServiceHandler(new AnonymousClass2(nonce));
    }

    public void cancel(byte[] nonce) {
        ContextService.runOnServiceHandler(new AnonymousClass3(nonce));
    }
