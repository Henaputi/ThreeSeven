...
public class ManagedVariableManager {
    ...
    class AnonymousClass2 implements Action1<T> {
    ...
    class AnonymousClass3 implements Action1<ManagedVariableModel> {
        final /* synthetic */ Class val$clazz;
        final /* synthetic */ Listener val$listener;
        final /* synthetic */ String val$tag;

        AnonymousClass3(String str, Listener listener, Class cls) {
            this.val$tag = str;
            this.val$listener = listener;
            this.val$clazz = cls;
        }
    ...
    class AnonymousClass5 implements Func1<ManagedVariableModel, Boolean> {
        final /* synthetic */ Class val$clazz;
        final /* synthetic */ String val$tag;
    ...
    static {
        sModelMap = new HashMap<Class<? extends ManagedVariable>, Class<? extends ManagedVariableModel>>() {
            {
    ...
    public <T extends ManagedVariable> UpsightSubscription fetch(Class<T> clazz, String tag, Listener<T> listener) {
        ...
        synchronized (this.mCache) {
            ...
            if (cachedVariable != null) {
                subscription = Subscriptions.from(Observable.just(cachedVariable).observeOn(this.mCallbackScheduler).subscribe(new AnonymousClass2(listener)));
            } else {
                subscription = Subscriptions.from(fetchDataStoreObservable(clazz, tag).subscribe(new AnonymousClass3(tag, listener, clazz), new AnonymousClass4(listener)));
            }
        }
        return subscription;
    }
    ...
    private <T extends ManagedVariable> Observable<? extends ManagedVariableModel> fetchDataStoreObservable(Class<T> clazz, String tag) {
        return this.mDataStore.fetchObservable((Class) sModelMap.get(clazz)).filter(new AnonymousClass5(clazz, tag)).defaultIfEmpty(null);
    }
